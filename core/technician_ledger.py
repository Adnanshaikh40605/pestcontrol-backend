...
def _economics(job: JobCard) -> str:
    if is_amc_economics(job):
        return 'amc'
    if is_contractual_economics(job):
        return 'contract'
    return 'one_time'
...
def _technician_share(job: JobCard, technician: Technician) -> Decimal:
    ...
    if participation is not None:
        return quantize_money(participation.payout_amount_snapshot)
...
def serialize_ledger_row(job: JobCard, technician: Technician) -> dict:
    ...
    if company_share <= 0 and completed and visit_revenue > 0:
    ...
    return {
        ...
        'planned_visits': job.planned_visit_count or job.max_cycle,
