...
def get_fcm_config_status() -> dict[str, Any]:
    """Diagnose partner FCM setup (no secrets)."""
    ...
    if not has_json and not has_split and not has_file:
        return {
            ...
            'reason': (
                'Missing Firebase credentials. Set FIREBASE_SERVICE_ACCOUNT_JSON on Railway '
    ...
    if _firebase_init_error:
        return {
            'configured': False,
            'project_id': project_id,
            'reason': f'Firebase credentials invalid: {_firebase_init_error}',
...
def _get_firebase_app():
    ...
    try:
        if sa_json:
            ...
            logger.info('Firebase Admin ready (JSON, project=%s)', project_id)
            return _firebase_app
...
def _prune_invalid_tokens(batch: list[str], responses) -> None:
    ...
    for idx, resp in enumerate(responses):
        if resp.success:
            continue
        if not isinstance(resp.exception, messaging.UnregisteredError):
            continue
        if idx >= len(batch):
            continue
        token = batch[idx]
...
def send_push_to_tokens(
    ...
    *,
    ...
    """Send FCM. data_only=True lets the app show local notification with custom sound."""
